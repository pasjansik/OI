...
inline void push(int w, int lw, int pw)
{
    ...
    {
        int dl = pw - lw + 1;
        ll wspol = (lazy[w].ent - lazy[w].beg) / (dl - 1);
        
        ll wartP = lazy[w].beg;
        lazy[l].beg += wartP;
        ll wartK = (dl / 2 - 1) * wspol + lazy[w].beg;
        lazy[l].ent += wartK;
    ...
    int dl = pw - lw + 1;
    ll wspol = (lazy2[w].beg - lazy2[w].ent) / (dl - 1);

    ll wartP = lazy2[w].beg;
    lazy2[l].beg += wartP;
    ll wartK = lazy2[w].ent + wspol * (dl / 2);
    lazy2[l].ent += wartK;
...
void update(int w, int lw, int pw, int lz, int pz, ll val, ll wspo, int flag)
{
    if(lw >= lz && pw <= pz)
    {
        int dl = pw - lw + 1;
        ll last = val + wspo * (dl - 1);
        tr[w] += (val + last) * dl / 2;
        if(flag == 0)
        {
            lazy[w].beg += val;
    ...
    push(w, lw, pw);
    int mid = (lw + pw) / 2;
    int iloscWLewym = max(0, mid - max(lz, lw) + 1);
    int iloscWPrawym = max(0, max(0, min(pz, pw) - (mid + 1) + 1));
...
ll query(int w, int lw, int pw, int lz, int pz)
{
    if(lw >= lz && pw <= pz)
        return tr[w];
    if(pw < lz || lw > pz)
